...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <h1 class="font-display text-4xl font-bold text-gray-900 mt-3">Edit Jurnal</h1>
        <p class="text-xs text-gray-400 font-semibold mt-1">
            ✍️ Edit catatan perasaanmu untuk menyempurnakan tulisan hari itu.
        </p>
    </div>

    {{-- Main Form Card --}}
    <form method="POST" action="{{ route('journal.update', $journal) }}" class="bg-white rounded-3xl p-6 md:p-8 border border-gray-100/70 shadow-sm space-y-7">
        @csrf @method('PUT')

        {{-- 1. Mood Picker --}}
        <div>
            <label class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-3.5">
                1. Bagaimana Perasaanmu Saat Itu? *
            </label>
        ...
        <div>
            ...
            <textarea id="content" name="content" rows="10"
                      class="w-full px-5 py-4 rounded-2xl border border-gray-150 focus:border-violet-300 focus:ring-4 focus:ring-violet-500/5 focus:outline-none text-sm text-gray-900 bg-gray-50/20 resize-none leading-relaxed transition-all duration-200 font-medium h-60">{{ old('content', $journal->content) }}</textarea>
            @error('content')
                <p class="mt-1.5 text-xs font-bold text-red-500">{{ $message }}</p>
            @enderror
        </div>
