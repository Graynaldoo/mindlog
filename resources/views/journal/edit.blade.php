...
<div class="max-w-3xl mx-auto animate-fade-up">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Jurnal</h1>
    ...
    {{-- Main Form Card --}}
    <form method="POST" action="{{ route('journal.update', $journal) }}" class="card p-6 md:p-8 space-y-8">
        @csrf @method('PUT')

        {{-- 1. Mood Picker --}}
        <div>
            <label class="input-label mb-4">
                Bagaimana Perasaanmu Saat Itu? <span class="text-rose-500">*</span>
            </label>
        ...
        <div>
            ...
            <textarea id="content" name="content" rows="10"
                      class="input resize-none h-64 leading-relaxed">{{ old('content', $journal->content) }}</textarea>
            @error('content')
                <p class="mt-2 text-xs font-bold text-rose-500">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex items-center gap-4 pt-6 border-t border-slate-100">
            ...
            <a href="{{ route('journal.show', $journal) }}" class="btn btn-secondary">
                Batalkan
            </a>
        </div>
    </form>
